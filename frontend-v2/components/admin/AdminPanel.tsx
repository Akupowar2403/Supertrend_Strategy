  ...
  return (
    <div style={{ padding: '24px 32px 40px', display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 16 }}>
        {STATS.map(s => {
          ...
          return (
            <div
              key={s.key}
              onClick={() => setActiveFilter(prev => prev === s.filter ? 'all' : s.filter)}
              style={{
                background: s.bg, borderRadius: 14, padding: '20px',
      ...
      <div style={{
        ...
        borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.3)',
      }}>
        <div style={{
