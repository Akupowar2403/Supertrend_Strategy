...
function getActions(
  ...
  const del         : ActionDef = { label: '🗑 Delete',       variant: 'rose',   onClick: onDelete                      }
  ...
  switch (effective) {
    case 'approve': return hasAdmin ? [revoke, removeAdmin, del] : [revoke, makeAdmin, del]
...
export default function UserRow({ user, activeFilter, onAssign, onRemove, onDelete }: Props) {
  ...
  const actions = getActions(user, activeFilter, onAssign, onRemove, () => setConfirmDelete(true))
      ...
      <td style={{ padding: '16px 22px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{
            ...
            fontSize: 16, fontWeight: 700, color: '#a5b4fc', border: '2px solid #3730a3',
          }}>
      ...
      <td style={{ padding: '16px 22px' }}>
        {confirmDelete ? (
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            >Yes, Delete</button>
            <button onClick={() => setConfirmDelete(false)} style={{ ...BTN.ghost, padding: '5px 12px' }}>Cancel</button>
          </div>
        ) : (
            ...
            {actions.map(a => (
              <button
                ...
                onMouseLeave={e => Object.assign(e.currentTarget.style, {
                  ...
                  borderColor: (BTN[a.variant] as { border?: string }).border,
                })}
              >{a.label}</button>
            ))}
          </div>
